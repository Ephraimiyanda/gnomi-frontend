...
export default function DebateWarRoomPage({ params }: { params: { id: string } }) {
  ...
  const handleVote = (votedForUserId: 'user-1-id' | 'user-2-id') => {
    if (!socketRef.current || !debateId) return
        ...
        <div ref={transcriptRef} className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
          {rounds.length === 0 ? (
            <div className="rounded-xl border border-dashed border-slate-700 p-6 text-sm text-slate-400">
              Waiting for first turn from debaters...
            </div>
          ) : (
            rounds.map((round) => {
              ...
              return (
                <div key={round.id} className={`max-w-[90%] rounded-xl p-4 text-sm leading-6 ${isUserOne ? 'border border-cyan-700/50 bg-cyan-900/20 text-cyan-100' : 'ml-auto border border-violet-700/50 bg-violet-900/20 text-violet-100'}`}>
      ...
      <aside className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
        ...
        {status === 'IN_PROGRESS' && (
          <div className="space-y-3">
            ...
            <button onClick={handleSubmitTurn} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-500 px-4 py-3 font-medium text-slate-950 hover:bg-cyan-400">
        ...
        {status === 'VOTING' && (
          <div className="space-y-3">
            ...
            <button onClick={() => handleVote('user-1-id')} className="w-full rounded-xl border border-cyan-700/60 bg-cyan-900/20 px-4 py-4 text-left hover:border-cyan-500">
              <p className="font-medium text-cyan-200">Vote User 1</p>
              <p className="text-sm text-cyan-400">{votesTally['user-1-id'] ?? 0} votes</p>
            </button>
            <button onClick={() => handleVote('user-2-id')} className="w-full rounded-xl border border-violet-700/60 bg-violet-900/20 px-4 py-4 text-left hover:border-violet-500">
        ...
        {status === 'FINISHED' && (
          <div className="space-y-3 rounded-2xl border border-amber-600/50 bg-amber-900/20 p-4">
            ...
            <p className="text-sm text-amber-100/90">{aiReport?.reasoning ?? 'Reasoning unavailable.'}</p>
            <div>
              ...
              <ul className="space-y-1 text-sm text-amber-100/90">
                {(aiReport?.fallaciesFound ?? ['None reported']).map((fallacy) => (

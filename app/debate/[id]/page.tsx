...
export default function DebateWarRoomPage({
  ...
  const { rounds, status, votesTally, aiReport, setDebateState } =
    useDebateStore();
        ...
        >
          {rounds.length === 0 ? (
            <div className="rounded-xl border border-dashed border-slate-700 p-6 text-sm text-slate-400">
              Transcript preview mode — socket flow intentionally disabled for
              now.
            </div>
          ) : (
            rounds.map((round) => {
              ...
              return (
                <div
                  key={round.id}
                  className={`max-w-[90%] rounded-xl p-4 text-sm leading-6 ${
                    isUserOne
                      ? "border border-cyan-700/50 bg-cyan-900/20 text-cyan-100"
                      : "ml-auto border border-violet-700/50 bg-violet-900/20 text-violet-100"
                  } ${status === "IN_PROGRESS" ? "animate-pulse" : ""}`}
                >
                  <p className="mb-2 text-xs uppercase tracking-wide opacity-75">
                    {isUserOne ? "User 1" : "User 2"} • Round{" "}
                    {round.roundNumber}
                  </p>
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-xl border border-slate-700 bg-slate-950 p-3">
              <textarea
                ...
                placeholder="Compose argument..."
                className="h-32 w-full resize-none bg-transparent text-sm text-slate-200 outline-none placeholder:text-slate-500"
              />
            </div>
        ...
        {status === "VOTING" && (
          <div className="space-y-3">
            <p className="text-sm text-slate-300">
              Vote for the stronger argument:
            </p>
        ...
        {status === "FINISHED" && (
          <div className="space-y-3 rounded-2xl border border-amber-600/50 bg-amber-900/20 p-4">
            ...
            <p className="text-sm text-amber-100/90">
              {aiReport?.reasoning ??
                "Reasoning appears here once backend finishes scoring."}
            </p>
            <div>
